...
public class UsuarioController {
    ...
    public ResponseEntity<?> azureLogin(@Valid @RequestBody com.arcadia.usuarios.dto.AzureAuthRequest request) {
        log.info("POST /azure-login");
        return usuarioService.loginConAzure(request.getIdToken())
                .<ResponseEntity<?>>map(ResponseEntity::ok)
                .orElse(ResponseEntity.status(HttpStatus.UNAUTHORIZED)
                        .body(Map.of("error", "Token de Microsoft Entra ID inválido o expirado")));
    }
